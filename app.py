...
#audio
@app.route('/audio_feed',methods=['POST','GET'])
def audioupload():
    if request.method == "POST":
        if  request.form.get('trans') == 'Transcribe':
            speechtotext() 
            return render_template('normal.html', transcript=text,data = frame_cnt , gram = grammermist , pau = pauses , arti = articulates , dur = duration , ros =rate_of_speech)
